...
while True:
    ...
    if event == 'sort':
        ...
        sim_scores = initializeSort(temp_df, similarMovies, typeToSort)
        if (typeToSort == "Most Similar"):
            finalSortedMovies = sim_scores[0:8]
        else:
            #quickSort()
            #mergeSort()
            print("Not done yet")
    
        for item in finalSortedMovies:
            print(df.iloc[item[0]]["title"])

        movie_indices = [i[0] for i in finalSortedMovies]
